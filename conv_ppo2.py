...
#Agent 1  (PPO, structure, explodation, optimizer
PPOAGENT = False
DQNAGENT = False

#Agent x Tenserflow Agent,  FullLayersOnly = False
TENSORFLOWAGENT = True

#Reward, evironment, rewardshaped, simple agent (offpolicy training)
...
if TENSORFLOWAGENT:
	CENTERED = True
	FULLLAYERSONLY = False
    
LAYERS = 10
if not CENTERED:
    LAYERS +=1  # own posistion layer
...
class CustomNetwork1(Network):

		def __init__(self, scope='CustomNetwork1', summary_labels=()):
...
################## TensorForce Agent from TensorFlow network ##################    
if TENSORFLOWAGENT:
                    
	# Create a Proximal Policy Optimization agent
	#network=[dict(type='conv2d', size=64, window=3, stride=1, padding='SAME', activation='relu'),
	agent = PPOAgent(
		states=dict(type='float', shape=(11,11,LAYERS)),
		actions=dict(type='int',	num_actions=env.action_space.n),
		batching_capacity=1000,
		network=CustomNetwork1, #CustomTFLayerNetwork,
		step_optimizer=dict(
			type='adam',
			learning_rate=1e-3
		)
...
if PPOAGENT:
	...
	#agent = DQNAgent(
		...
		network=[dict(type='conv2d', size=64, window=3, stride=1, padding='SAME', activation='relu'),
				 ...
				 dict(type='conv2d', size=64, window=3, stride=1, padding='SAME', activation='relu'),
				 dict(type='flatten'),
				 dict(type='dense', size = 3000),
				 dict(type='dense', size = 1000),        
...
if DQNAGENT:
 #DQN agent in tensorforce
	agent = DQNAgent(
			states=dict(type='float', shape=(11,11,LAYERS)),
			actions=dict(type='int', num_actions=env.action_space.n),
			network=[dict(type="conv2d", size=64, window=4, stride=2), dict(type='flatten'), dict(type='dense', size = 64)],
			batching_capacity=1000,
	)
...
class RewardShaping_Simple:
	def __init__(self):
		self.sa = SimpleAgent(config["agent"](3, config["game_type"]))				
...
def new_featurize(obs, center=False, FullLayersOnly = True):
...
class WrappedEnv(OpenAIGym):
	...
	def execute(self, action):
		...
		agent_reward, test_action = self.rewardShaping_Simple.shape_it(obs[self.gym.training_agent], action)
		action = test_action
		...
		obs = self.gym.get_observations()
		#obscopy =	copy.deepcopy(obs[self.gym.training_agent]) Observation is changed by featurize
		...
		agent_state = new_featurize(state[self.gym.training_agent],CENTERED)
        
				# calculate envioenmnt reward NEEDED 
		#agent_reward = reward[self.gym.training_agent]
				
		# REWARD shaping: logging rewards
		if (terminal):
				self.ecount += 1
				...
				#pom-x reward shaping
		#agent_reward, reward_info = self.rewardShaping.shape_it(obs[self.gym.training_agent], action, agent_reward)
				#simple agent based shaping
				
				#TESTING, need to be re-entered 
		#agent_reward, test_action = self.rewardShaping_Simple.shape_it(obscopy, action)				
	...
	def reset(self):
		...
		agent_obs = new_featurize(obs[3],CENTERED)
		return agent_obs
...
runner.run(episodes=1000000, max_episode_timesteps=2000, episode_finished=episode_finished)
