...
# reward shaping:
SAREWARD = False # reward shaping using Simple Agent (Off policy, Simple agent in control)
...
#Tensorflow Wrightmann net (scale inputs to -1 to 1),  rigid + wood walls  (0.5, 1), no eps limit, random start
##Writghtman junk paste
#def _rescale(x):
#    return (x - 0.5) * 2.0
...
#random shuffle
#if RANDOMSTART: 
#    order = list(range(len(AOrder))); random.shuffle(order)
#    AOrder =  list(np.array(AOrder)[order])
...
# un-shuffle to get correct ordering.
#if RANDOMSTART: 
#    l_out = [0] * len(agents)
...
class WrappedEnv(OpenAIGym):
	...
	def execute(self, action):
        ...
        #Simple agent reward, Off policy    SAREWARD
		...
		agent_state = new_featurize(state[self.gym.training_agent])
